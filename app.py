...
# Set BUSGO_ADMIN_USER and BUSGO_ADMIN_PASSWORD before first run.
# Defaults are intentionally absent — app will prompt if not set.
ADMIN_USERNAME = os.environ.get("BUSGO_ADMIN_USER", "").strip()
ADMIN_PASSWORD = os.environ.get("BUSGO_ADMIN_PASSWORD", "").strip()

# Session timeout in seconds (30 minutes idle)
...
def gen_pnr() -> str:
    """Cryptographically random 8-digit PNR."""
    return "BG" + "".join(secrets.choice("0123456789") for _ in range(8))

def check_session_timeout():
    """Expire sessions after SESSION_TIMEOUT seconds of inactivity."""
    last = st.session_state.get("last_active", 0)
    if st.session_state.get("logged_in") and (time.time() - last) > SESSION_TIMEOUT:
        logout()
...
def is_locked_out(username: str) -> tuple[bool, int]:
    ...
    if attempts >= MAX_LOGIN_ATTEMPTS and elapsed < LOCKOUT_SECONDS:
        return True, int(LOCKOUT_SECONDS - elapsed)
    if elapsed >= LOCKOUT_SECONDS:
        st.session_state.pop(key, None)  # reset after timeout
    return False, 0

def record_failed_login(username: str):
    key = f"lockout_{username}"
    data = st.session_state.get(key, (0, time.time()))
    attempts = data[0] + 1
    st.session_state[key] = (attempts, data[1] if data[0] > 0 else time.time())
...
# ─── DB ──────────────────────────────────────────────────────────────────────
def get_conn():
    # One persistent connection per Streamlit session via session_state cache
...
def _seed_admin(conn):
    """Seed the admin account only if env vars are configured."""
    if not ADMIN_USERNAME or not ADMIN_PASSWORD:
        return  # skip seeding — admin must be configured via env vars
    c = conn.cursor()
...
def _seed_buses(conn):
    ...
    if c.fetchone()[0] == 0:
        ...
        c.executemany(
            "INSERT INTO buses(bus_name,bus_number,from_city,to_city,departure,arrival,total_seats,price,bus_type,amenities) VALUES(?,?,?,?,?,?,?,?,?,?)",
            buses
        )
        conn.commit()
...
def authenticate(username, password):
    ...
    if row and check_pw(password, row[2]):
        return (row[0], row[1])
    return None
...
def register_user(full_name, username, password, mobile, email):
    ...
    try:
        c.execute(
            "INSERT INTO users(full_name,username,password,role,mobile,email) VALUES(?,?,?,?,?,?)",
...
def logout():
    for k in ["logged_in","username","role","page","selected_bus","selected_seats",
              "search_results","last_active"]:
...
def search_buses(fc, tc):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "SELECT id,bus_name,bus_number,from_city,to_city,departure,arrival,total_seats,price,bus_type,amenities FROM buses WHERE from_city=? AND to_city=? AND is_active=1",
        (fc, tc)
    )
...
def get_booked_seats(bus_id, jdate):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "SELECT seat_number FROM bookings WHERE bus_id=? AND journey_date=? AND status='Confirmed'",
        (bus_id, jdate)
    )
...
def create_booking(username, bus_id, pname, page, pgender, seat, jdate, fare, discount, total_paid, coupon, payment):
    ...
    c.execute(
        "INSERT INTO bookings(pnr,username,bus_id,passenger_name,passenger_age,passenger_gender,seat_number,journey_date,fare,discount,total_paid,coupon_used,payment_method,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,'Confirmed')",
...
def get_user_bookings(username):
    ...
    c.execute("""SELECT b.pnr,bu.bus_name,bu.from_city,bu.to_city,b.passenger_name,b.seat_number,
        ...
        FROM bookings b JOIN buses bu ON b.bus_id=bu.id WHERE b.username=? ORDER BY b.booking_date DESC""",
        (username,)
    )
    rows = c.fetchall(); conn.close(); return rows
...
def cancel_booking(pnr, username, is_admin=False):
    conn = get_conn(); c = conn.cursor()
    if is_admin:
        c.execute("SELECT total_paid,status FROM bookings WHERE pnr=?", (pnr,))
    else:
        c.execute("SELECT total_paid,status FROM bookings WHERE pnr=? AND username=?", (pnr, username))
    row = c.fetchone()
...
def generate_pdf(info: dict) -> bytes:
    ...
    orange = colors.HexColor("#f97316"); card = colors.HexColor("#1a1f35")
    muted = colors.HexColor("#7c8ab0"); white = colors.white
    ...
    tbl_style = TableStyle([
        ...
        ("ROWBACKGROUNDS",(0,0),(-1,-1),[card, colors.HexColor("#1f2540")]),
        ("GRID",(0,0),(-1,-1),0.5,colors.HexColor("#252d4a")),
        ("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7),("LEFTPADDING",(0,0),(-1,-1),8)
    ])
    data = [
        [Paragraph("Bus",lbl_s),Paragraph(info['bus_name'],val_s),Paragraph("Bus No.",lbl_s),Paragraph(info['bus_number'],val_s)],
    ...
    p_data = [
        [Paragraph("Passenger",lbl_s),Paragraph(info['passenger_name'],val_s),Paragraph("Age/Gender",lbl_s),Paragraph(f"{info.get('passenger_age','')} / {info.get('passenger_gender','')}",val_s)],
    ...
    elems.append(Spacer(1, 3*mm))
    fare_data = [
        [Paragraph("Base Fare",lbl_s), Paragraph(f"Rs. {info['fare']:.2f}",val_s)],
    ...
    f_tbl.setStyle(TableStyle([
        ("BACKGROUND",(0,0),(-1,-1),card),("BACKGROUND",(0,2),(-1,2),colors.HexColor("#1a2a1a")),
        ("GRID",(0,0),(-1,-1),0.5,colors.HexColor("#252d4a")),
        ("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7),("LEFTPADDING",(0,0),(-1,-1),8)
    ]))
...
# ─── AUTH PAGE ────────────────────────────────────────────────────────────────
def render_auth():
    # Warn if admin env vars are missing
    if not ADMIN_USERNAME or not ADMIN_PASSWORD:
        st.warning(
            "⚠️ **Admin account not configured.** "
            "Set `BUSGO_ADMIN_USER` and `BUSGO_ADMIN_PASSWORD` environment variables before deploying. "
            "Run: `export BUSGO_ADMIN_USER=yourname BUSGO_ADMIN_PASSWORD=yourpassword`"
        )
    ...
    with col2:
                ...
                if not uname or not passwd:
                    ...
                    if locked:
                        ...
                        if res:
                            ...
                            _, remaining = is_locked_out(uname)
                            attempts_left = max(0, MAX_LOGIN_ATTEMPTS - st.session_state.get(f"lockout_{uname}", (0,0))[0])
                            if attempts_left > 0:
        ...
        with tab_r:
            ...
            r_pass = st.text_input("Password *", type="password", key="rp", placeholder="Min 8 characters")
            ...
            if st.button("Create Account  →", use_container_width=True, key="btn_reg"):
                ...
                elif len(r_user.strip()) < 3: errors.append("Username must be ≥ 3 characters.")
                elif username_exists(r_user.strip()): errors.append("Username already taken.")
                if not r_pass: errors.append("Password is required.")
                elif len(r_pass) < 8: errors.append("Password must be ≥ 8 characters.")
...
def render_sidebar():
    with st.sidebar:
        ...
        user = escape(st.session_state.get("username", ""))
        ...
        st.markdown(f"""<div style='padding:.5rem;text-align:center;'>
            ...
            <span class='badge {bc}'>{escape(role)}</span></div>""", unsafe_allow_html=True)
        st.divider()
        ...
        remaining_s = max(0, SESSION_TIMEOUT - idle)
        mins, secs = divmod(remaining_s, 60)
        st.caption(f"⏱️ Session expires in {mins}m {secs}s")
...
def page_book():
    ...
    if not results: st.info(f"No buses from **{escape(from_c)}** to **{escape(to_c)}**."); return
        ...
        with st.container():
            st.markdown(f"""<div class='bus-card'>
                <div style='display:flex;justify-content:space-between;align-items:flex-start;'>
                    <div><div style='font-family:Syne,sans-serif;font-size:1.15rem;font-weight:700;'>{escape(bname)}</div>
                    <div style='color:#7c8ab0;font-size:.85rem;'>{escape(bnum)} &nbsp;·&nbsp; <span class='badge badge-blue'>{escape(btype)}</span></div></div>
                ...
                <div style='display:flex;gap:2rem;margin-top:.8rem;'>
                    <div><div style='font-size:1.1rem;font-weight:700;'>{escape(dep)}</div><div style='color:#7c8ab0;font-size:.8rem;'>{escape(fc)}</div></div>
                    <div style='color:#7c8ab0;font-size:1.2rem;padding-top:4px;'>→</div>
                    <div><div style='font-size:1.1rem;font-weight:700;'>{escape(arr)}</div><div style='color:#7c8ab0;font-size:.8rem;'>{escape(tc)}</div></div>
                </div>
                <div style='margin-top:.6rem;font-size:.82rem;color:#7c8ab0;'>
                    🪑 {avail} seats available &nbsp;·&nbsp; {escape(amenities).replace(","," · ")}
...
def _booking_flow(jdate: str):
    ...
    st.markdown(f"### 🪑 Select Seat — {escape(bname)}")
    ...
    with st.form("bform"):
        ...
        if cu in COUPONS:
            ...
            discount = round(subtotal * cval / 100, 2) if ctype == "percent" else min(float(cval), subtotal)
            coupon_v = cu
        total_paid = max(0, subtotal - discount)
        st.markdown(f"""<div class='bus-card' style='margin-top:.5rem;'>
            <div style='display:flex;justify-content:space-between;'><span>Seats</span><span>{escape(', '.join(seat_choice))}</span></div>
        ...
        if st.form_submit_button("✅ Confirm Booking", use_container_width=True):
            if not pname.strip():
                ...
                for seat in seat_choice:
                    pnr = create_booking(
                        st.session_state.username, bus_id, pname.strip(), page, pgender,
...
def page_my_bookings():
    ...
    if is_admin:
        ...
        for r in rows:
            ...
            with st.expander(f"🎫 {escape(pnr)}  ·  {escape(fc)} → {escape(tc)}  ·  {jdate}  ·  {escape(user)}"):
                st.markdown(f"""<div style='display:flex;gap:2rem;flex-wrap:wrap;'>
                    <div><b>Bus</b><br>{escape(bname)}</div><div><b>Passenger</b><br>{escape(pname)}</div>
                    <div><b>Seat</b><br>{escape(seat)}</div><div><b>Paid</b><br>₹{paid:.2f}</div>
                    <div><b>Payment</b><br>{escape(pay)}</div>
                    <div><b>Status</b><br><span class='badge {badge}'>{escape(status)}</span></div>
        ...
        for r in rows:
            ...
            badge = "badge-green" if status=="Confirmed" else "badge-red"
            with st.expander(f"🎫 {escape(pnr)}  ·  {escape(fc)} → {escape(tc)}  ·  {jdate}"):
                st.markdown(f"""<div class='bus-card'>
                    <div style='display:flex;justify-content:space-between;'>
                        <div><div style='font-family:Syne,sans-serif;font-weight:700;font-size:1.05rem;'>{escape(bname)}</div>
                        <div style='color:#7c8ab0;font-size:.85rem;'>{escape(dep)} → {escape(arr)} &nbsp;·&nbsp; {escape(btype)}</div></div>
                        <span class='badge {badge}'>{escape(status)}</span>
                    </div>
                    <div style='display:flex;gap:2rem;flex-wrap:wrap;margin-top:.8rem;'>
                        <div><b>Passenger</b><br>{escape(pname)}</div><div><b>Seat</b><br>{escape(seat)}</div>
                        <div><b>Date</b><br>{jdate}</div><div><b>Paid</b><br>₹{paid:.2f}</div>
                        <div><b>Payment</b><br>{escape(pay)}</div>
                        {f"<div><b>Coupon</b><br>{escape(coupon)}</div>" if coupon else ""}
                        {f"<div><b>Refund</b><br>₹{refund:.2f}</div>" if refund else ""}
                    </div></div>""", unsafe_allow_html=True)
    ...
    if st.button("Search", use_container_width=True, key="btn_pnr"):
        ...
        st.markdown(f"""<div class='bus-card' style='margin-top:1rem;'>
            <div style='display:flex;justify-content:space-between;align-items:center;'>
                <div style='font-family:Syne,sans-serif;font-size:1.3rem;font-weight:800;'>{escape(pnr)}</div>
                <span class='badge {badge}'>{escape(status)}</span>
            </div><hr style='border-color:#252d4a;margin:.8rem 0;'>
            <div style='display:grid;grid-template-columns:1fr 1fr;gap:.8rem;'>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>BUS</div><div>{escape(bname)} ({escape(bnum)})</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>TYPE</div><div>{escape(btype)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>FROM → TO</div><div>{escape(fc)} → {escape(tc)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>DEPARTURE / ARRIVAL</div><div>{escape(dep)} / {escape(arr)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>JOURNEY DATE</div><div>{jdate}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>SEAT</div><div>{escape(seat)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>PASSENGER</div><div>{escape(pname)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>AGE / GENDER</div><div>{page2} / {escape(pgender)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>BASE FARE</div><div>₹{paid+disc:.2f}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>DISCOUNT</div><div style='color:#22c55e;'>- ₹{disc:.2f} {escape(coupon or '')}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>TOTAL PAID</div><div style='color:#f97316;font-weight:700;'>₹{paid:.2f}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>PAYMENT</div><div>{escape(pay)}</div></div>
                <div><div style='color:#7c8ab0;font-size:.8rem;'>BOOKED BY</div><div>{escape(user)}</div></div>
...
def page_cancel():
    ...
    if st.button("Proceed to Cancel", use_container_width=True, key="btn_cancel"):
        ...
        if ok: st.success(f"✅ Booking **{escape(pnr)}** cancelled. Refund of **₹{val:.2f}** will be processed."); st.balloons()
        else: st.error(val)
...
def page_admin():
    ...
    with t_users:
        ...
        if not users: st.info("No users.")
            ...
            for u in users:
                ...
                with st.expander(f"{icon} {escape(uname)}  ·  {escape(urole)}"):
                    c1, c2 = st.columns([3,1])
                    with c1:
                        st.markdown(f"""<div style='font-size:.9rem;'>
                            <div><b>Full Name:</b> {escape(fname or '—')}</div>
                            <div><b>Mobile:</b> {escape(mobile or '—')}</div>
                            <div><b>Email:</b> {escape(email or '—')}</div>
    ...
    with t_buses:
        ...
        for b in get_all_buses():
            ...
            with st.expander(f"{'✅' if active else '🔴'} {escape(bname)}  ·  {escape(fc)} → {escape(tc)}"):
                c1, c2 = st.columns([3,1])
                with c1:
                    st.markdown(f"""<div style='font-size:.9rem;display:grid;grid-template-columns:1fr 1fr;gap:.5rem;'>
                        <div><b>Bus No:</b> {escape(bnum)}</div><div><b>Type:</b> {escape(btype)}</div>
                        <div><b>Departure:</b> {escape(dep)}</div><div><b>Arrival:</b> {escape(arr)}</div>
                        <div><b>Seats:</b> {seats}</div><div><b>Price:</b> ₹{price:.0f}</div>
                        <div><b>Amenities:</b> {escape(amenities)}</div>
                    </div>""", unsafe_allow_html=True)
                with c2:
    ...
    with t_add:
        ...
        with st.form("abf"):
            c1, c2 = st.columns(2)
            with c1:
                ab_name = st.text_input("Bus Name *",  max_chars=100)
                ab_num  = st.text_input("Bus Number *", max_chars=20)
                ab_from = st.text_input("From City *",  max_chars=50)
                ab_to   = st.text_input("To City *",    max_chars=50)
                ab_dep  = st.text_input("Departure *",  placeholder="HH:MM", max_chars=5)
            with c2:
                ab_arr   = st.text_input("Arrival *",  placeholder="HH:MM", max_chars=5)
                ...
                ab_am    = st.text_input("Amenities", placeholder="WiFi,Water,Blanket", max_chars=200)
            if st.form_submit_button("Add Bus", use_container_width=True):
                if not all([ab_name, ab_num, ab_from, ab_to, ab_dep, ab_arr]):
                    st.error("Fill all required fields.")
                elif not re.fullmatch(r"\d{2}:\d{2}", ab_dep) or not re.fullmatch(r"\d{2}:\d{2}", ab_arr):
                    st.error("Departure and Arrival must be in HH:MM format.")
                else:
                    add_bus(ab_name, ab_num, ab_from, ab_to, ab_dep, ab_arr, int(ab_seats), float(ab_price), ab_type, ab_am)
                    st.success(f"Bus '{escape(ab_name)}' added!"); st.rerun()
    ...
    with t_stats:
        ...
        if top:
            ...
            for route, cnt in top: st.markdown(f"- **{escape(route)}** → {cnt} bookings")
